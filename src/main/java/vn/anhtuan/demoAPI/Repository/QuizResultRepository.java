...
public interface QuizResultRepository extends JpaRepository<QuizResult, Integer> {
    ...
    // ====== Aggregate theo NGÀY từ quiz_results ======
    @Query(value = """
    ...
    // ====== Tổng kỳ (tính trực tiếp từ quiz_results) ======
    @Query(value = """
    ...
    // (2) Trung bình cộng theo NGÀY
    // QuizResultRepository.java

    @Query(value = """
    SELECT
      COALESCE(SUM(qr.correct_answers), 0) AS correct_sum,
      COALESCE(SUM(qr.total_questions), 0) AS total_sum,
      MAX(qr.completed_at)                 AS last_time
    FROM quiz_results qr
    JOIN quizzes q ON q.id = qr.quiz_id
    WHERE qr.user_id = :userId
      AND qr.status = 'COMPLETED'
      ...
      AND (:quizTypeId IS NULL OR q.quiz_type_id = :quizTypeId)
""", nativeQuery = true)
    List<Object[]> aggregateOverallByUserAndFilters(@Param("userId") Long userId,
                                                    @Param("gradeId") Integer gradeId,
                                                    @Param("subjectId") Integer subjectId,
                                                    @Param("quizTypeId") Integer quizTypeId);
